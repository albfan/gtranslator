...
#define GTR_PANEL_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), GTR_TYPE_PANEL, GtranslatorPanelPrivate))

struct _GtranslatorPanelPrivate 
{
...
typedef struct _GtranslatorPanelItem GtranslatorPanelItem;

struct _GtranslatorPanelItem 
{
	gchar *name;
	GtkWidget *icon;
};
...
static void	 gtranslator_panel_set_property	(GObject         *object,
						 guint            prop_id,
						 const GValue    *value,
						 GParamSpec      *pspec);
...
G_DEFINE_TYPE(GtranslatorPanel, gtranslator_panel, GTK_TYPE_VBOX)

static void
gtranslator_panel_finalize (GObject *obj)
{
	g_object_unref (GTR_PANEL (obj)->priv->tooltips);

	if (G_OBJECT_CLASS (gtranslator_panel_parent_class)->finalize)
		(*G_OBJECT_CLASS (gtranslator_panel_parent_class)->finalize) (obj);
}
...
static void
gtranslator_panel_close (GtranslatorPanel *panel)
{
...
gtranslator_panel_focus_document (GtranslatorPanel *panel)
{
	...
	if (GTK_WIDGET_TOPLEVEL (toplevel) && GTR_IS_WINDOW (toplevel))
   	{
		GtranslatorView *view;

		view = gtranslator_window_get_active_view (GTR_WINDOW (toplevel));
		if (view != NULL)
...
gtranslator_panel_grab_focus (GtkWidget *w)
{
	gint n;
	GtkWidget *tab;
	GtranslatorPanel *panel = GTR_PANEL (w);
...
static void
gtranslator_panel_class_init (GtranslatorPanelClass *klass)
{
	GtkBindingSet *binding_set;
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	g_type_class_add_private (klass, sizeof (GtranslatorPanelPrivate));

	object_class->constructor = gtranslator_panel_constructor;
	object_class->finalize = gtranslator_panel_finalize;
	object_class->get_property = gtranslator_panel_get_property;
	object_class->set_property = gtranslator_panel_set_property;
	...
	signals[ITEM_ADDED] =
		g_signal_new ("item_added",
			      ...
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (GtranslatorPanelClass, item_added),
			      NULL, NULL,
	...
	signals[ITEM_REMOVED] =
		g_signal_new ("item_removed",
			      G_OBJECT_CLASS_TYPE (klass),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (GtranslatorPanelClass, item_removed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[CLOSE] =
		g_signal_new ("close",
			      ...
			      G_STRUCT_OFFSET (GtranslatorPanelClass, close),
		  	      NULL, NULL,
...
static void
sync_title (GtranslatorPanel     *panel,
	    GtranslatorPanelItem *item)
{
...
static void
panel_show (GtranslatorPanel *panel,
	    gpointer    user_data)
{
	gint page;
	GtkNotebook *nb;
...
static void
gtranslator_panel_init (GtranslatorPanel *panel)
{
	panel->priv = GTR_PANEL_GET_PRIVATE (panel);
...
static void
build_horizontal_panel (GtranslatorPanel *panel)
{
	GtkWidget *box;
	GtkWidget *sidebar;
...
static void
build_vertical_panel (GtranslatorPanel *panel)
{
	GtkWidget *close_button;
			       ...
			       GObjectConstructParam *construct_properties)
{
	...
	GtranslatorPanelClass *klass = GTR_PANEL_CLASS (g_type_class_peek (GTR_TYPE_PANEL));
...
GtkWidget *
gtranslator_panel_new (GtkOrientation orientation)
{
	return GTK_WIDGET (g_object_new (GTR_TYPE_PANEL, "orientation", orientation, NULL));
}

static GtkWidget *
build_tab_label (GtranslatorPanel  *panel,
		 GtkWidget   *item,
		 const gchar *name,
		 GtkWidget   *icon)
{
...
void
gtranslator_panel_add_item_with_stock_icon (GtranslatorPanel  *panel, 
				      GtkWidget   *item, 
				      const gchar *name,
				      const gchar *stock_id)
{
	...
	gtranslator_panel_add_item (panel, item, name, icon);
}
			       ...
			       GtkWidget  *item)
{
	...
	g_return_val_if_fail (GTR_IS_PANEL (panel), FALSE);
...
GtkOrientation
gtranslator_panel_get_orientation (GtranslatorPanel *panel)
{
	g_return_val_if_fail (GTR_IS_PANEL (panel), GTK_ORIENTATION_VERTICAL);

	return panel->priv->orientation;
}

gint
gtranslator_panel_get_n_items (GtranslatorPanel *panel)
{
	g_return_val_if_fail (GTR_IS_PANEL (panel), -1);

	return gtk_notebook_get_n_pages (GTK_NOTEBOOK (panel->priv->notebook));
}

gint
_gtranslator_panel_get_active_item_id (GtranslatorPanel *panel)
{
	gint cur_page;
	GtkWidget *item;
	GtranslatorPanelItem *data;

	g_return_val_if_fail (GTR_IS_PANEL (panel), 0);
